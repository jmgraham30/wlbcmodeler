...
infection_freq_rf_rmu_sim <- function(rep_num,F_val_m,F_cv,mu_vect,bin_props,N_val,...){

  a_sim <- infection_freq_rf_rmu_iteration(F_val_m,F_cv,mu_vect,bin_props,N_val)
  sim_res <- extract_stats(a_sim)
  parm_data <- tibble::tibble(rep_num=rep_num,F_val_m=F_val_m,F_cv=F_cv,mu_vect=mu_vect,bin_props=bin_props,N_val=N_val)
